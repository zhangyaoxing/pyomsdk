...
def test_snapshots_get_all_cluster(
...
def test_snapshots_get_one_cluster(
    client: OpsManagerClient, project_with_cluster: dict[str, Any]
) -> None:
    cluster_id = _first_cluster_id(client, project_with_cluster["id"])
    if not cluster_id:
    ...
    all_result = resource.get_all_cluster(
        SnapshotsResource.GetAllClusterPathParams(
            project_id=project_with_cluster["id"], cluster_id=cluster_id
        ),
        None,
    )
    items = all_result.get("results", [])
    if not items:
    ...
    path_params = SnapshotsResource.GetOneClusterPathParams(
        project_id=project_with_cluster["id"], cluster_id=cluster_id, snapshot_id=snapshot_id
    )
